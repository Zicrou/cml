...
import './style.css'
import { createRelationShipLanguage } from '../../redux/actions/RelationShipLanguage/create'
...
const RelationShipLanguage: React.FC = (props: any) => {
  const { history } = props
  const { questionId } = props.match.params
  const relationshipAnswers = props.location.state && props.location.state.relationshipAnswers
  const [isAnswerSelected, setIsAnswerSelected] = useState(false)
  const createRelationShipLanguageStatus = useSelector((state: RootState) => state.createRelationShipLanguage)
  ...
  const onSubmitUpdate = (event: any) => {
    ...
    if (formDataObj.options) {
      ...
      const currentAnswer = relationshipAnswers.filter(
        (answer: { questionId: number; sortId: number }) => answer.questionId === 13 && answer.sortId === 1
      )
      dispatch(
        updateAnswer({
    ...
    if (formDataObj.comment) {
      const currentAnswer = relationshipAnswers.filter(
        (answer: { questionId: number; sortId: number; response: string }) =>
          answer.questionId === 13 && answer.sortId === 2 && answer.response !== formDataObj.comment
      )
  ...
  useEffect(() => {
    dispatch(getAnswerBack('relationship-language'))
    setLoader(true)
  }, [dispatch])

  useEffect(() => {
    if (createRelationShipLanguageStatus && createRelationShipLanguageStatus.status == 200) {
      const apiCounter = answerApiResponseCounter + 1
      setAnswerApiResponseCounter(apiCounter)
      if (answerApiCounter === apiCounter) {
        setLoader(false)
        window.location.replace('/relationship-marriage')
      }
    }
  }, [createRelationShipLanguageStatus, answerApiCounter, history])

  useEffect(() => {
    if (questionId) {
      const answers = relationshipAnswers.filter((answer: { questionId: number }) => answer.questionId === 13)
  ...
  const onSubmit = (event: any) => {
    ...
    if (back != undefined && back != '') {
      ...
      if (formDataObj.options && formDataObj.comment != '') {
        setLoader(true)
        counter += 2
        dispatch(createRelationShipLanguage({ sortId: '1', response: formDataObj.options }))
        dispatch(createRelationShipLanguage({ sortId: '2', response: formDataObj.comment }))
      }
      ...
      if (formDataObj.options && formDataObj.comment == '') {
        ...
        counter += 2
        dispatch(createRelationShipLanguage({ sortId: '1', response: formDataObj.options }))
        dispatch(createRelationShipLanguage({ sortId: '2', response: '' }))
      }
            ...
            <div className="col-md-6">
              {questionId ? (
                <div className="cml-logo-title">
                  <div className="languages-cml-logo">
              ...
              <div className="languages-header-text">
                <div className="languages-sub-heading-1">
                  <b>Relationship!</b>
                </div>
                <div className="languages-sub-heading-2">
                  <p>
                    Are you comfortable with a situation in which either you or your spouse speaks a language that the
                    other does not understand, with his/her friends or family?
                  </p>
                </div>
              </div>
              <Form onSubmit={questionId ? onSubmitUpdate : onSubmit}>
                <div className="languages-section">
                  <div className="languages-btn-group btn-group-toggle languages-radiobtns" data-toggle="buttons">
                    <div className="items col-md-12">
                      <div className="first-option">
                        <label
                          className={
                            isActive.isYes
                              ? 'btn btn-default languages-custom-radio-btn active'
                              : 'btn btn-default languages-custom-radio-btn'
                          }
                          onChange={() => {
                            setIsActive({
                      ...
                      <div className="languages-form-group">
                        <div className="details-textarea">
                          <textarea
                            ...
                            cols={66}
                            name="comment"
                            className="languages-details-textarea"
                            placeholder="Details"
                    ...
                    {isAnswerSelected === true ? (
                      <div className="language-items col-md-12" style={{ marginTop: '10px' }}>
                    ...
                    {loader ? (
                      ...
                      <div className="languages-btns">
                        <a href="/relationship-spiritual-contribution">
                          <input type="button" value="Back" className="back-button-style" />
                        </a>
                        <button className="languages-continue-btn">{questionId ? 'Update' : 'Continue'}</button>
                      </div>
                    )}
                  </div>
                </div>
...
export default withRouter(RelationShipLanguage)
