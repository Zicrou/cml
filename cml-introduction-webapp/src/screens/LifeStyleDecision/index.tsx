...
import { createLifeStyleDecision } from '../../redux/actions/lifeStyleDecision/create'
...
const LifeStyleDecision: React.FC = (props: any) => {
  const { history } = props
  ...
  const createLifeStyleDecisionStatus = useSelector((state: RootState) => state.createLifeStyleDecision)
  ...
  useEffect(() => {
    dispatch(getAnswerBack('lifestyle-decision'))
    setLoader(true)
  }, [dispatch])
  ...
  const onSubmitUpdate = (event: any) => {
    ...
    if (formDataObj.comment) {
      ...
      const currentAnswer = lifestyleAnswers.filter(
        (answer: { questionId: number; sortId: number }) => answer.questionId === 4
      )
      dispatch(
        updateAnswer({
  ...
  useEffect(() => {
    if (questionId) {
      const answers = lifestyleAnswers.filter((answer: { questionId: number }) => answer.questionId === 4)
  ...
  useEffect(() => {
    if (createLifeStyleDecisionStatus && createLifeStyleDecisionStatus.status === 200) {
      setLoader(false)
      window.location.replace('/lifestyle-health')
    }
  }, [createLifeStyleDecisionStatus, history])
  ...
  const onSubmit = (event: any) => {
    ...
    if (back != undefined && back != '') {
      ...
      if (formDataObj.comment) {
        setLoader(true)
        dispatch(createLifeStyleDecision({ sortId: '1', response: formDataObj.comment }))
      }
    }
  }
            ...
            <div className="col-md-6">
              ...
              <div className="decision-header-text">
                ...
                <div className="decision-sub-heading-2">
                  <p>How do you make big and small decisions in your life?</p>
                </div>
              </div>
...
export default withRouter(LifeStyleDecision)
