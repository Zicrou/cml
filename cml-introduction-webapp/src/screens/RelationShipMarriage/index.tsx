...
import './style.css'
import { createRelationShipMarriage } from '../../redux/actions/RelationShipMarriage/create'
...
const RelationShipMarriage: React.FC = (props: any) => {
  const { history } = props
  const { questionId } = props.match.params
  const relationshipAnswers = props.location.state && props.location.state.relationshipAnswers
  const createRelationShipMarriageStatus = useSelector((state: RootState) => state.createRelationShipMarriage)
  ...
  const onSubmit = (event: any) => {
    ...
    if (back != undefined && back != '') {
      ...
      if (formDataObj.options && formDataObj.comment != '') {
        setLoader(true)
        counter += 2
        dispatch(createRelationShipMarriage({ sortId: '1', response: formDataObj.options }))
        dispatch(createRelationShipMarriage({ sortId: '2', response: formDataObj.comment }))
      }
      ...
      if (formDataObj.options && formDataObj.comment == '') {
        ...
        counter += 2
        dispatch(createRelationShipMarriage({ sortId: '1', response: formDataObj.options }))
        dispatch(createRelationShipMarriage({ sortId: '2', response: '' }))
      }
  ...
  const onSubmitUpdate = (event: any) => {
    ...
    if (formDataObj.options) {
      ...
      const currentAnswer = relationshipAnswers.filter(
        (answer: { questionId: number; sortId: number }) => answer.questionId === 14 && answer.sortId === 1
      )
      dispatch(
        updateAnswer({
    ...
    if (formDataObj.comment) {
      const currentAnswer = relationshipAnswers.filter(
        (answer: { questionId: number; sortId: number; response: string }) =>
          answer.questionId === 14 && answer.sortId === 2 && answer.response !== formDataObj.comment
      )
  ...
  useEffect(() => {
    dispatch(getAnswerBack('relationship-marriage'))
    setLoader(true)
  }, [dispatch])
  ...
  useEffect(() => {
    if (updateAnswerStatus && updateAnswerStatus.status == 200) {
      ...
      if (back != undefined) {
        window.location.replace('/relationship-endscreen')
      }
      if (relationshipAnswers != undefined) {
        window.location.replace('/profile')
      }
    }
  }, [updateAnswerStatus, answerApiCounter, history, dispatch])
  useEffect(() => {
    if (questionId) {
      const answers = relationshipAnswers.filter((answer: { questionId: number }) => answer.questionId === 14)
      for (let i = 0; i < answers.length; i++) {
        if (answers[i].sortId === 1) {
          setIsActive({
            isYes: answers[i].response == 'yes' ? true : false,
  ...
  useEffect(() => {
    if (createRelationShipMarriageStatus && createRelationShipMarriageStatus.status == 200) {
      const apiCounter = answerApiResponseCounter + 1
      setAnswerApiResponseCounter(apiCounter)
      if (answerApiCounter === apiCounter) {
        setLoader(false)
  ...
  }, [createRelationShipMarriageStatus, answerApiCounter, history])
  return (
    <>
            ...
            <div className="col-md-6">
              ...
              <div className="marriages-header-text">
                ...
                <div className="marriages-sub-heading-2">
                  <p>Are you comfortable with an inter-ethnicity marriage?</p>
                </div>
              </div>
                ...
                <div className="marriages-section">
                  <div className="marriages-btn-group btn-group-toggle radiobtns" data-toggle="buttons">
                    <div className="items col-md-12">
                      <div className="first-option">
                        <label
                          className={
                            isActive.isYes
                              ? 'btn btn-default marriages-custom-radio-btn active'
                              : 'btn btn-default marriages-custom-radio-btn'
                          }
                          onChange={() => {
                            setIsActive({
                    ...
                    {isAnswerSelected === true ? (
                      <div className="marriage-items col-md-12" style={{ marginTop: '10px' }}>
                    ...
                    {loader ? (
                      <div className="marriages-btns">
                        <Loader />
                      </div>
                    ) : (
                      <div className="marriages-btns">
                        <a href="/relationship-language">
                          <input type="button" value="Back" className="back-button-style" />
                        </a>
                        <button className="marriages-continue-btn">{questionId ? 'Update' : 'Continue'}</button>
                      </div>
                    )}
                  </div>
                </div>
...
export default withRouter(RelationShipMarriage)
